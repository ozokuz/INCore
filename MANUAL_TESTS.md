...
- [ ] Given a placed `Card Vendor` and enough `Card Token` items for an offer, when buying that offer from any page, then only tokens are consumed and the configured booster/box reward is granted.
- [ ] Given a placed `Card Vendor`, token shortfall, and enough Numismatics bank SPUR balance to cover conversion at `8 SPUR = 1 TOKEN`, when opening vendor UI, then the offer stays purchasable and its cost panel shows horizontal token/spur icon+count entries for the shortfall (without a separate `Cost` text label).
- [ ] Given a placed `Card Vendor`, token shortfall, and enough Numismatics bank SPUR balance to cover conversion, when clicking `Buy`, then a conversion confirmation prompt appears in exchange-layout style (source/target amounts with arrow), contains no amount-adjustment buttons, and shows adjacent buttons with `Cancel` on the left and `Convert & Buy` on the right.
- [ ] Given a placed `Card Vendor`, token shortfall, and enough Numismatics bank SPUR balance to cover conversion, when viewing that offer cost strip, then no red `Card Token` missing-count entry is shown.
- [ ] Given the conversion confirmation prompt is open for a vendor offer, when clicking `Convert & Buy`, then the purchase succeeds and exactly `missing_tokens * 8` SPUR is deducted from the Numismatics bank balance plus remaining required tokens are consumed.
- [ ] Given the conversion confirmation prompt is open for a vendor offer, when clicking `Cancel`, then no currency is consumed and no reward item is granted.
- [ ] Given a placed `Card Vendor` with token shortfall and not enough Numismatics bank SPUR balance to cover conversion, when opening vendor UI, then the offer `Buy` button is disabled.
- [ ] Given a placed `Card Vendor` with booster and booster-box offers visible, when viewing the offer rows, then each row renders its corresponding item icon (`Card Booster` or `Card Booster Box`) alongside the offer text.
- [ ] Given a placed `Card Vendor` is open, when viewing the top balance section, then it renders only currency item icons with counts (`Card Token` and bank `SPUR`) in cost-strip style and does not show text labels for those balances.
- [ ] Given a placed `Card Vendor` is opened in-world, when viewing the screen background behind the vendor panel, then the world scene remains unblurred while vendor UI text and buttons remain legible.
- [ ] Given vendor offer datapack entries define `stock_min` and `stock_max`, when placing/opening a fresh `Card Vendor`, then each listed offer starts with stock inside its configured inclusive min/max range.
- [ ] Given two separately placed fresh `Card Vendor` blocks, when opening each once, then their rolled offer stocks are tracked independently (buying from one does not reduce the other).
- [ ] Given a vendor offer has remaining stock greater than 1, when increasing buy quantity with `+` and purchasing once, then reward amount and currency cost are applied for that selected quantity and stock decreases by exactly that quantity.
...
- [ ] Given a player is near a card vendor and op permissions are available, when running `/incore cards vendor_open <player>`, then the nearest vendor opens for that player and uses that block's own stock state.
