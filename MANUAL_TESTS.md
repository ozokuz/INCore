...
## Card System
- [ ] Given op permissions, when running `/incore cards give_booster <player> incore:base_protocol` and right-clicking the `Card Booster`, then one pack opens, a results screen appears, and exactly five `Card Module` items are added or dropped.
...
- [ ] Given a cryptic card in inventory from `/incore cards give_module <player> incore:base_cryptic_shell 1`, when right-clicking a placed `Decryptor` with empty hands, then that card becomes revealed and decrypt success message is shown.
...
- [ ] Given no deck equipped and inventory contains a core, box, and module cards from `/incore cards give_core`, `/incore cards give_deck_box`, and `/incore cards give_module`, when right-clicking a placed `Deck Station` with empty hands, then one `Deck` item is created and source parts are consumed.
...
- [ ] Given a placed `Card Vendor`, card tokens in inventory, and optional `numismatics:spur`, when right-clicking vendor with empty hands and buying an offer, then currency is consumed and configured booster/box reward is granted.
