...
- [ ] Given a placed `Card Vendor`, card tokens in inventory, and optional `numismatics:spur`, when right-clicking vendor with empty hands and buying an affordable offer from any page in the paged offer list, then currency is consumed and configured booster/box reward is granted.
- [ ] Given a placed `Card Vendor` and insufficient currency for at least one offer, when opening vendor UI, then unaffordable offers show disabled `Buy` buttons and affordable offers remain purchasable.
