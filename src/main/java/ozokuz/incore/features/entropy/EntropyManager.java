...
public final class EntropyManager {
    private static final String KEY_CURRENT = "incore:entropy_current";
    private static final String KEY_CAP_BONUS = "incore:entropy_cap_bonus";
    private static final String KEY_LAST_UPDATE_MS = "incore:entropy_last_update_ms";
    ...
    public static void update(ServerPlayer player) {
        ...
        if (!data.contains(KEY_LAST_UPDATE_MS)) {
            ...
            if (!data.contains(KEY_CURRENT)) {
                data.putInt(KEY_CURRENT, getEntropyCap(player));
            }
            clampToCap(player);
            return;
        }

        int cap = getEntropyCap(player);
    ...
    public static int getCurrentEntropy(ServerPlayer player) {
        update(player);
        return player.getPersistentData().getInt(KEY_CURRENT);
    }

    public static void setCurrentEntropy(ServerPlayer player, int value) {
        update(player);
    ...
    public static void addEntropy(ServerPlayer player, int delta) {
        if (delta == 0) {
            return;
        }
        ...
        if (delta > 0) {
            BattlePassTaskHooks.onEntropyRecovered(player, delta);
        }
    }

    public static int getEntropyCap(ServerPlayer player) {
        int baseCap = Config.ENTROPY_BASE_CAP.get();
    ...
    public static int getEntropyCapBonus(ServerPlayer player) {
        return Math.max(0, player.getPersistentData().getInt(KEY_CAP_BONUS));
    }

    public static void setEntropyCapBonus(ServerPlayer player, int bonus) {
        player.getPersistentData().putInt(KEY_CAP_BONUS, Math.max(0, bonus));
        clampToCap(player);
    }

    public static void addEntropyCapBonus(ServerPlayer player, int extra) {
        if (extra <= 0) {
            return;
        }

        int currentBonus = getEntropyCapBonus(player);
        long nextBonus = (long) currentBonus + extra;
        setEntropyCapBonus(player, (int) Math.min(Integer.MAX_VALUE, nextBonus));
    }

    public static void adjustEntropyCapBonus(ServerPlayer player, int delta) {
        if (delta == 0) {
            return;
        }
        ...
        setEntropyCapBonus(player, clamped);
    }
    ...
    public static long getRegenIntervalMillis() {
        return Math.max(1L, Config.ENTROPY_REGEN_INTERVAL_SECONDS.get()) * 1000L;
    }
