...
public class PowerInputMenu extends AbstractContainerMenu {
    ...
    public PowerInputMenu(int containerId, Inventory playerInventory, BlockPos blockPos) {
        ...
        addDataSlot(slot(
                () -> blockEntity instanceof IMachinePowerInput input ? input.family().ordinal() : -1,
                value -> familyOrdinal = value
        ));
        addDataSlot(slot(
                () -> blockEntity instanceof IMachinePowerInput input ? input.powerTier() : 0,
        ...
        addDataSlot(slot(
                () -> blockEntity instanceof IMachinePowerInput input ? input.availablePower(Integer.MAX_VALUE) : 0,
    ...
    private void syncFromBlockEntity(BlockEntity blockEntity) {
        if (blockEntity instanceof IMachinePowerInput input) {
            ...
            availablePower = input.availablePower(Integer.MAX_VALUE);
        } else {
            familyOrdinal = -1;
            powerTier = 0;
            availablePower = 0;
    ...
    public boolean stillValid(Player player) {
        if (!(player.level().getBlockEntity(blockPos) instanceof IMachinePowerInput)) {
            return false;
        }
    ...
    public MachinePowerFamily family() {
        return familyOrdinal < 0 || familyOrdinal >= MachinePowerFamily.values().length
                ? null
                : MachinePowerFamily.values()[familyOrdinal];
    }
