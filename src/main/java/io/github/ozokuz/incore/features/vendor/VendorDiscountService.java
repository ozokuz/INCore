...
public final class VendorDiscountService {
    ...
    public static boolean hasDiscountCharmEquipped(ServerPlayer player) {
        return CuriosApi.getCuriosInventory(player)
                .map(handler -> handler.findFirstCurio(stack -> stack.is(Registration.VENDOR_DISCOUNT_CHARM_ITEM.get())).isPresent())
                .orElse(false);
    }

    public static int curioBonusChancePercent() {
        return Math.clamp(Config.VENDOR_DISCOUNT_CURIO_BONUS_CHANCE_PERCENT.get(), 0, 100);
    }

    public static int curioBonusAmountPercent() {
        return Math.clamp(Config.VENDOR_DISCOUNT_CURIO_BONUS_AMOUNT_PERCENT.get(), 0, 100);
    }

    public static int baseDiscountMinPercent() {
        return Math.clamp(Config.VENDOR_OFFER_DISCOUNT_MIN_PERCENT.get(), 0, 100);
    }

    public static int baseDiscountMaxPercent() {
        return Math.clamp(Config.VENDOR_OFFER_DISCOUNT_MAX_PERCENT.get(), 0, 100);
    }

    private static DiscountRollProfile baseProfile() {
        int chance = Math.clamp(Config.VENDOR_OFFER_DISCOUNT_CHANCE_PERCENT.get(), 0, 100);
        int chanceCap = Math.clamp(Config.VENDOR_OFFER_DISCOUNT_CHANCE_CAP_PERCENT.get(), 0, 100);
        int minPercent = Math.clamp(Config.VENDOR_OFFER_DISCOUNT_MIN_PERCENT.get(), 0, 100);
        int maxPercent = Math.clamp(Config.VENDOR_OFFER_DISCOUNT_MAX_PERCENT.get(), 0, 100);
