...
import io.github.ozokuz.incore.features.researchv2.station.PowerInputMenu;
...
public class PowerInputScreen extends StationStatusScreen<PowerInputMenu> {
    ...
    protected int accentColor() {
        return menu.family() == ResearchPowerFamily.MECHANICAL ? 0xFF9B6A39 : 0xFF4A8DB2;
    }
    ...
    protected void renderStatusBody(GuiGraphics guiGraphics, int left, int top, int mouseX, int mouseY) {
        ...
        if (menu.family() == ResearchPowerFamily.MECHANICAL) {
    ...
    private static Component familyText(ResearchPowerFamily family) {
        if (family == null) {
